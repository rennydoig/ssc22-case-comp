...
def w_avg2(df, group_col, to_avg, to_w_sum=None, to_sum=None, weight='tests'):
    ...
    for i, group in enumerate(groups): # for each ID
        ...
        for t in conn_types:
            sub_idx = (df[group_col]==group) & (df['conn_type'] == t)
            group_df = df[sub_idx].copy()
            ...
            for year in years:
                ...
                for quarter in quarters:
                    ...
                    # extract a group                
                    time_idx = (group_df['year'] == year) & (group_df['quarter'] == quarter)
                    
                    time = str(year) + '-' + str(quarter)
                    
                    temp_table = group_df.loc[time_idx, :]
                    temp_weight = temp_table[weight] / np.sum(temp_table[weight]) # compute the weights
                    ...
                    # append the averaged value to the result dataframe
                    result_df = result_df.append(dict(zip(result_cols, new_row)), ignore_index=True)
